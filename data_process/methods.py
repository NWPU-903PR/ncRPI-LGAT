...
def reset_basic_data(interaction_list, negative_interaction_list, lncRNA_list, protein_list):
    ...
    for i in range(len(interaction_list)):
        ...
        temp_interaction = LncRNA_Protein_Interaction(lncRNA=lncRNA_list[lncRNA_index],
                                                      protein=protein_list[protein_index], y=1)
    ...
    for i in range(len(negative_interaction_list)):
        ...
        temp_interaction = LncRNA_Protein_Interaction(lncRNA=lncRNA_list[lncRNA_index],
                                                      protein=protein_list[protein_index], y=0)
...
def Accuracy_Precision_Sensitivity_Specificity_MCC(model, loader, device):
    ...
    FP = 0
    FN = 0
    interaction_list = []                      #add interaction_list
    for data in loader:
        if torch.cuda.device_count() > 1:
            data = data
        else:
            data = data.to(device)
        prediction = model(data).max(dim=1)
        pred = prediction[1]   # pred wei suo you hang zui da de weizhi
        pred_values = prediction[0]

        for index in range(len(pred)):
            if torch.cuda.device_count() > 1:
                target = torch.cat([i.y.long() for i in data]).to(device)
                interaction_name = ([i.edge_attr for i in data])
            else:
                target = data.y
                interaction_name = data.edge_attr                              ####add interaction_name


            if pred[index] == 1 and target[index] == 1:
                TP += 1
            elif pred[index] == 1 and target[index] == 0:
                FP += 1


                interaction_name[index].append(pred_values[index].tolist())
                list = copy.copy(interaction_name[index])
                interaction_list.append(list)
                interaction_name[index].pop()

            elif pred[index] == 0 and target[index] == 1:
                FN += 1
            else:
                TN += 1
    ...
    if (FP + TN) != 0:
        ...
        Specificity = 0
    return Accuracy, Precision, Sensitivity, Specificity, MCC, interaction_list
