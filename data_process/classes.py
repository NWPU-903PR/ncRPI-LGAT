...
class LncRNA_Protein_Interaction_dataset_1hop_1220_InMemory(InMemoryDataset):
    def __init__(self, root, interaction_list=None, h=None, set_allInteractionKey_forGenerate = None, set_allInteractionKey_cannotUse = None, set_allinteractionKey_forline =None, transform=None, pre_transform=None):
        self.interaction_list = interaction_list
        self.h = h
        self.sum_node = 0.0
        ...
        self.set_allinteractionKey_forline = set_allinteractionKey_forline
    ...
    def local_subgraph_generation(self, interaction, h):
            ...
            if interaction_key not in self.set_allInteractionKey_cannotUse:
                ...
                if interaction_temp.lncRNA.serial_number not in nodeSerialNumber_subgraphNodeSerialNumber_dict.keys():
                    ...
                    for i in RNA_proterinlist:####################################################################
                        if (interaction_temp.lncRNA.serial_number, i) in self.set_allinteractionKey_forline:
        ...
        interaction_name = [interaction.lncRNA.name, interaction.protein.name]                     ###############################

        # 用x,y,edge_index创建出data，加入存放data的列表local_subgraph_list
        x = torch.tensor(x, dtype=torch.float)
        y = torch.tensor(y, dtype=torch.float)
        edge_index = torch.tensor(edge_index, dtype=torch.long)
        data = Data(x=x, y=y, edge_index=edge_index, interaction_name=interaction_name)           #interaction_name=interaction_name

        return data


class LncRNA_Protein_Interaction_dataset_1hop_1222_InMemory(InMemoryDataset):
    def __init__(self, root, interaction_list=None, h=None, set_allInteractionKey_forGenerate=None,
                 set_allInteractionKey_cannotUse=None, transform=None, pre_transform=None):
        self.interaction_list = interaction_list
        self.h = h
        self.sum_node = 0.0
        self.set_allInteractionKey_forGenerate = set_allInteractionKey_forGenerate
        self.set_allInteractionKey_cannotUse = set_allInteractionKey_cannotUse
        self.path = root + '/nodelist'
        super(LncRNA_Protein_Interaction_dataset_1hop_1222_InMemory, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
        self.subgraphNodeSerialNumber_node_list = torch.load(self.path)
    ...
    def process(self):
        ...
        if self.interaction_list != None and self.h != None:
            ...
            for interaction in self.interaction_list:
                interaction_key = (interaction.lncRNA.serial_number, interaction.protein.serial_number)
                if interaction_key in self.set_allInteractionKey_forGenerate:
                    data, subgraphNodeSerialNumber_node_dict = self.local_subgraph_generation(interaction, self.h)
                    data_list.append(data)
            ...
            print(f'average node number = {self.sum_node / count}')
            torch.save((data, slices), self.processed_paths[0])
            torch.save(subgraphNodeSerialNumber_node_list, self.path)
    ...
    def local_subgraph_generation(self, interaction, h):
        ...
        for interaction_temp in interaction.lncRNA.interaction_list:
            ...
            if interaction_key not in self.set_allInteractionKey_cannotUse:
                ...
                if interaction_temp.protein.serial_number not in nodeSerialNumber_subgraphNodeSerialNumber_dict.keys():
                    nodeSerialNumber_subgraphNodeSerialNumber_dict[
                        interaction_temp.protein.serial_number] = subgraph_serialNumber
                    subgraphNodeSerialNumber_node_dict[subgraph_serialNumber] = interaction_temp.protein
                    subgraph_serialNumber += 1
        ...
        for interaction_temp in interaction.protein.interaction_list:
            ...
            if interaction_key not in self.set_allInteractionKey_cannotUse:
                ...
                if interaction_temp.lncRNA.serial_number not in nodeSerialNumber_subgraphNodeSerialNumber_dict.keys():
                    nodeSerialNumber_subgraphNodeSerialNumber_dict[
                        interaction_temp.lncRNA.serial_number] = subgraph_serialNumber
                    subgraphNodeSerialNumber_node_dict[subgraph_serialNumber] = interaction_temp.lncRNA
                    subgraph_serialNumber += 1

        set_interactionSerialNumberPair_wait_to_add.remove(
            (interaction.lncRNA.serial_number, interaction.protein.serial_number))
        set_interactionSerialNumberPair_wait_to_add = [(interaction.lncRNA.serial_number,
                                                        interaction.protein.serial_number)] + list(
            set_interactionSerialNumberPair_wait_to_add)
        ...
        return data, subgraphNodeSerialNumber_node_dict
